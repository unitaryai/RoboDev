...
#   OPENAI_API_KEY     — OpenAI API key (required, injected via secret)
#
# Exit codes:
#   0 — success
#   1 — failure
...
EXIT_CODE=0
codex --quiet --approval-mode full-auto "${TASK_PROMPT}" > /workspace/codex-output.txt 2>&1 || EXIT_CODE=$?

# ---- Write result ----
if [[ ${EXIT_CODE} -eq 0 ]]; then
    OUTPUT="$(cat /workspace/codex-output.txt 2>/dev/null || echo '')"
